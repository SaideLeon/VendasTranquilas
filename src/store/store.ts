...
interface AppState {
  ...
  // Product actions
  addProduct: (productData: Omit<Product, 'id' | 'createdAt' | 'userId'>) => Promise<Product | null>;
  updateProduct: (product: Product) => Promise<Product | null>;
  deleteProduct: (productId: string) => Promise<void>;
  getProductById: (productId: string) => Product | undefined; // Make sync

  // Sale actions
  addSale: (saleData: Omit<Sale, 'id' | 'profit' | 'productName' | 'createdAt' | 'userId'>) => Promise<Sale | null>;
  deleteSale: (saleId: string) => Promise<void>;
  getSaleById: (saleId: string) => Sale | undefined; // Make sync

  // Debt actions
  addDebt: (debtData: Omit<Debt, 'id' | 'createdAt' | 'status' | 'amountPaid' | 'userId'>) => Promise<Debt | null>;
  updateDebt: (debtId: string, updates: Partial<Omit<Debt, 'id' | 'createdAt' | 'userId'>>) => Promise<Debt | null>;
